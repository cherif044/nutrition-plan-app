...
function testReadyMealUiWithFoodCustomization() {
  ...
  assert(plannerHtml.includes('edit-mode-toggle'), 'planner should show an edit mode switch');
  assert(plannerHtml.includes('meal-add-food-btn'), 'planner should show add-food control');
  assert(plannerHtml.includes('meal-add-tray" hidden'), 'add-food control should be hidden before edit mode');
  ...
  assert(appJs.includes('state.editModeEnabled ?'), 'ingredient controls should only render in edit mode');
  ...
  assert(styles.includes('.edit-mode-switch'), 'edit mode should use switch styling');
